...
static 
std::ostream &emphasize(std::ostream &oss, std::string const &message, ANSI_ESCAPE em, std::string const &begin, std::string const &end, bool keep_quotes, bool start_at_word_boundary, bool force_no_ansi) {
    for(size_t qb = (start_at_word_boundary? find_after_word_boundary(message, begin): message.find_first_of(begin)),
        ...
        if(qb != std::string::npos && qe != std::string::npos) {
            if(!force_no_ansi) oss << em;
            if(keep_quotes) 
                oss << message.substr(qb, qe-qb+1); 
            else
                oss << message.substr(qb+1, qe-qb-1);
            if(!force_no_ansi && em != ANSI_NONE)
                oss << ANSI_RESET;
        }
    }
    return oss;
}
std::string emphasize(std::string const &message, ANSI_ESCAPE em, std::string const &begin, std::string const &end, bool keep_quotes, bool start_at_word_boundary, bool force_no_ansi) {
    std::ostringstream oss;
    emphasize(oss, message, em, begin, end, keep_quotes, start_at_word_boundary, force_no_ansi);
    return oss.str();
}
std::ostream &emphasize(std::ostream &oss, std::string const &message, ANSI_ESCAPE em, std::string const &begin, std::string const &end, bool keep_quotes, bool start_at_word_boundary) {
    return emphasize(oss, message, em, begin, end, keep_quotes, start_at_word_boundary, false);
}
