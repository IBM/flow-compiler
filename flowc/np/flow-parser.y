...
%nonassoc ID STRING INTEGER FLOAT SYMBOL NODE CONTAINER ENTRY IMPORT DEFINE OPTION OUTPUT RETURN ERROR MOUNT ENVIRONMENT INPUT HEADERS INIT LIMITS.
%left SEMICOLON.
%left DOT.
%left AT.
%left COMMA.
...
%extra_argument { struct fc::ast *ast }
...
stmt(A) ::= ID(B) valx(C) SEMICOLON.                           { A = ast->stmt_kwd(B) == FTK_IMPORT? ast->node(FTK_IMPORT, C): ast->node(ast->stmt_kwd(B), B, C); 
                                                                 ast->expect(A, {FTK_DEFINE, FTK_IMPORT}, "import directive or variable definition expected"); 
                                                                 if(ast->stmt_kwd(B) == FTK_DEFINE) {
                                                                    //ast->define_var(ast->get_id(B), C);
                                                                    ast->description.cat(true, "\n", A, {B, C});
                                                                 }
                                                               } 
stmt(A) ::= IMPORT(B) STRING(C) SEMICOLON.                        { A = ast->nappend(B, C); }
stmt(A) ::= INPUT(B) STRING(C) SEMICOLON.                         { A = ast->nappend(B, C); }
stmt(A) ::= CONTAINER(B) STRING(C) SEMICOLON.                         { A = ast->nappend(B, C); }

stmt(A) ::= ID(B) eqsc valx(C) SEMICOLON.                      { A = ast->node(ast->stmt_kwd(B), B, C); 
                                                                 ast->expect(A, FTK_DEFINE, "keyword used as variable name");
                                                                 //ast->define_var(ast->get_id(B), C);
                                                               }
stmt(A) ::= ID(B) ID(C) eqsc valx(D) SEMICOLON.                { 
                                                                 A = ast->node(ast->stmt_kwd(B), C, D); 
                                                                 ast->expect(A, FTK_OPTION, "option keyword expected here");
                                                               }
// input
stmt(A) ::= ID(B) blck(C).                                     { A = ast->node(ast->stmt_kwd(B), B, C);
...
stmt(A) ::= ID(B) dtid(C) blck(D).                             { A = ast->node(ast->stmt_kwd(B), B, C, D); 
                                                                 ast->expect(A, {FTK_CONTAINER, FTK_NODE, FTK_ENTRY, FTK_INPUT}, 
                                                                 "expected \"node\", \"entry\", \"input\" or \"container\" here");
...
// node with condition
stmt(A) ::= ID(B) dtid(C) OPENPAR fldr(E) CLOSEPAR blck(D).    { A = ast->node(ast->stmt_kwd(B), B, C, D, E); 
...
// error check 
stmt(A) ::= ID(B) OPENPAR fldr(C) CLOSEPAR fldr(D).            { A = ast->node(ast->stmt_kwd(B), C, D);
                                                                 ast->expect(A, FTK_ERROR, "expected \"error\" keyword");  
                                                                 ast->description.cat(true, "\n", A, {B});
                                                               }
stmt(A) ::= ID(B) OPENPAR fldr(C) CLOSEPAR valc(D) COMMA fldr(E). { A = ast->node(ast->stmt_kwd(B), C, D, E);
                                                                 ast->expect(A, FTK_ERROR, "expected \"error\" keyword");  
...
elem(A) ::= ID(B) lblk(C).                                     { ast->chtype(C, ast->blck_kwd(B));
                                                                 A = ast->node(FTK_elem, B, C); 
...
elem(A) ::= ID(B) oexp(C) SEMICOLON.                           { ast->chtype(C, ast->blck_kwd(B));
                                                                 A = ast->node(FTK_elem, B, C);
                                                                 ast->expect(C, {FTK_RETURN, FTK_OUTPUT}, "expected \"output\" or \"return\" here"); 
                                                               }          
elem(A) ::= ID(B) rexp(C) SEMICOLON.                           { ast->chtype(C, ast->blck_kwd(B));
                                                                 A = ast->node(FTK_elem, B, C); 
...
valn(A) ::= MINUS valn(B).                                     { A = B; /*ast->negate(B);*/ } [UMINUS]
...
valx(A) ::= valn(B).                                           { A = B; }
valx(A) ::= DOLLAR ID(C).                                      { 
                                                                 A = ast->find(0, FTK_DEFINE, ast->get_id(C)); 
                                                                 if(A == 0) 
                                                                    ast->error(C, stru1::sfmt() << "reference to undefined symbol \"" << ast->get_id(A=C) << "\""); 
                                                                 else 
                                                                    ast->refcount.update(A, ast->refcount(A)+1);
                                                               }
