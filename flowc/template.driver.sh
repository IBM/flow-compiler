...
echo "        Set the port the GRPC server is mapped locally. The default is $grpc_PORT"
echo ""
echo "    --rest-port PORT (or set {{NAME/id/upper}}_REST_PORT)"
echo "        Set the port the REST API is mapped locally. The default is $rest_PORT"
echo ""
echo "    --htdocs DIRECTORY (or set {{NAME/id/upper}}_HTDOCS)"
echo "        This is a Docker Composer only option. Mount DIRECTORY inside the container to use as a custom application."
