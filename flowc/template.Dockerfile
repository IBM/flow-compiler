...
RUN flowc --client --server {{MAIN_FILE}} --name {{NAME}} --grpc-certificate "{{GRPC_CERTIFICATE}}" --rest-certificate "{{REST_CERTIFICATE}}"  && make -j2 DBG=${DEBUG_IMAGE} -f {{NAME}}.mak deploy
WORKDIR /home/worker/{{NAME}}
